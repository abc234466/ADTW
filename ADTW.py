        ...
        if len(comparedIncreasingQueue) != 0 and matrix_ADTW[ts1_length - 1][ts2_length - 1] < comparedIncreasingQueue[0][0]:
            ...
            if ts1_index + 1 < ts1_length and ts2_index + 1 < ts2_length:
                ...
                if current_minimum_distance + distance_dia < matrix_ADTW[ts1_index + 1][ts2_index + 1]:
                    matrix_ADTW[ts1_index + 1][ts2_index + 1] = current_minimum_distance + distance_dia
            ...
            if ts1_index + 1 < ts1_length:
                ...
                if current_minimum_distance + distance_down < matrix_ADTW[ts1_index + 1][ts2_index]:
                    matrix_ADTW[ts1_index + 1][ts2_index] = current_minimum_distance + distance_down
            ...
            if ts2_index + 1 < ts2_length:
                ...
                if current_minimum_distance + distance_right < matrix_ADTW[ts1_index][ts2_index + 1]:
                    matrix_ADTW[ts1_index][ts2_index + 1] = current_minimum_distance + distance_right
            ...
            ts1_index = comparedIncreasingQueue[0][1][0]
            ts2_index = comparedIncreasingQueue[0][1][1]
            firstQueue = comparedIncreasingQueue[0]

            #remove duplicate cells
            while firstQueue in comparedIncreasingQueue:
                comparedIncreasingQueue.remove(firstQueue)
...
def updateQueue(queue, inputValue, ts1_index, ts2_index):
    ...
    if queue:
        if inputValue >= queue[len(queue)-1][0]:
            ...
            return queue
    # append value at the first time
    else:
        queue.append([inputValue, [ts1_index, ts2_index]])
        return queue
